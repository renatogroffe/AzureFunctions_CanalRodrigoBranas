...
namespace FunctionAppProcessarAcoes
{
    public class ProcessarAcoes
    {
        ...
        public void Run([QueueTrigger("queue-acoes", Connection = "AzureWebJobsStorage")] string myQueueItem,
            FunctionContext context)
        {
            var logger = context.GetLogger("ProcessarAcoes");
            logger.LogInformation($"Dados recebidos: {myQueueItem}");

            DadosAcao dadosAcao = null;
            try
            {
                dadosAcao = JsonSerializer.Deserialize<DadosAcao>(myQueueItem,
